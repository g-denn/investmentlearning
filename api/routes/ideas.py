...
from api.services.performance_backfill import refresh_idea_performance

router = APIRouter()


def _build_performance_response(performance: Performance) -> PerformanceResponse:
    """Convert a performance row into the frontend response shape."""
...
@router.get("/ideas/random", response_model=IdeaDetailResponse)
def get_random_idea(db: Session = Depends(get_db)):
    """
    Get a random investment idea that has performance data and a description.
    Returns full detail without performance (fetch separately after user decision).
    """
    try:
        idea = (
            db.query(Idea)
            .join(Performance, Idea.id == Performance.idea_id)
            .join(Description, Idea.id == Description.idea_id)
        ...
        if not idea:
            raise HTTPException(status_code=404, detail="No ideas with performance data found")

        if idea.link is None:
            idea.link = ""

        description = db.query(Description).filter(Description.idea_id == idea.id).first()
        catalysts = db.query(Catalysts).filter(Catalysts.idea_id == idea.id).first()
...
def get_idea_detail(idea_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        try:
            ...
            if performance:
                result.performance = _build_performance_response(performance)
        except Exception as e:
...
def get_idea_performance(
    ...
    refresh: bool = Query(
        False,
        description="Refresh performance from Yahoo Finance before returning cached values.",
    ),
    db: Session = Depends(get_db),
):
    """
    ...
    try:
        ...
        if refresh:
            try:
                refresh_result = refresh_idea_performance(db, idea_id=idea_id)
                if refresh_result and refresh_result.metrics is not None:
                    performance = db.query(Performance).filter(Performance.idea_id == idea_id).first()
            except Exception as e:
                # Live refresh should not break the UI if cached data already exists.
                print(f"Error refreshing performance data from Yahoo Finance: {e}")
        ...
        return _build_performance_response(performance)
    except HTTPException:
        # Re-raise HTTP exceptions without modification
        raise
    except Exception as e:
