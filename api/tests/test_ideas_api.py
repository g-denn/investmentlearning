...
from api.routes import ideas as ideas_routes
...
def test_get_idea_performance_refreshes_from_yahoo_when_requested(client, test_data, monkeypatch):
    """Test that the performance endpoint can refresh live data before returning."""
    idea_id = test_data["ideas"][0].id

    def fake_refresh(session, *, idea_id: str):
        performance = session.query(Performance).filter(Performance.idea_id == idea_id).first()
        performance.oneYearPerf = 2.25
        session.commit()
        return SimpleNamespace(metrics={"oneYearPerf": 2.25})

    monkeypatch.setattr(ideas_routes, "refresh_idea_performance", fake_refresh)

    response = client.get(f"/ideas/{idea_id}/performance?refresh=true")
    assert response.status_code == status.HTTP_200_OK
    assert response.json()["oneYearPerf"] == 2.25

def test_get_idea_performance_uses_cached_data_when_refresh_fails(client, test_data, monkeypatch):
    """Test that cached data is still returned if Yahoo refresh fails."""
    idea_id = test_data["ideas"][0].id
    ...
    response = client.get(f"/ideas/{idea_id}/performance?refresh=true")
    assert response.status_code == status.HTTP_200_OK
    assert response.json()["oneYearPerf"] == 1.5
